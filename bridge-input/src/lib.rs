...
pub fn request_accessibility_permission() {
    unsafe {
        extern "C" { fn AXIsProcessTrustedWithOptions(options: *const std::ffi::c_void) -> bool; }
...
#[cfg(target_os = "linux")]
pub fn request_accessibility_permission() {
    tracing::info!("On Linux, ensure /dev/uinput is accessible (add user to 'input' group)");
}

/// Convert macOS key code to string (for debugging, used by protocol)
pub fn keycode_to_string(keycode: u16) -> &'static str {
    // Keep the full existing keycode table here — it's the protocol format
    match keycode {
        ...
        0x35 => "Escape", 0x36 => "RightCommand", 0x37 => "Command",
        0x38 => "Shift", 0x39 => "CapsLock", 0x3A => "Option",
        0x3B => "Control", 0x3C => "RightShift", 0x3D => "RightOption",
        0x3E => "RightControl", 0x3F => "Function",
        0x7A => "F1", 0x78 => "F2", 0x63 => "F3", 0x76 => "F4",
        0x60 => "F5", 0x61 => "F6", 0x62 => "F7", 0x64 => "F8",
        0x65 => "F9", 0x6D => "F10", 0x67 => "F11", 0x6F => "F12",
        0x7B => "LeftArrow", 0x7C => "RightArrow",
